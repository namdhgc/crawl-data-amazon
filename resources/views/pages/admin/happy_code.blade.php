...
@section('title')
	<title>{{ Lang::get('menu.happy-code') }}</title>
@endsection
...
@section('js')
    ...
    <script src="{{ URL::asset('js/manage/happy_code/happy_code.js') }}" type="text/javascript"></script>
	...
	<div class="row">
        ...
        <div class="col-md-12">
            ...
            <div class="portlet box blue" id="search-tool">
                ...
                <div class="portlet-body"  style="display: block;">
                    ...
                    <form action="{{ URL::Route('auth-get-happy-code') }}" method="GET" id="search-tool" >
                    ...
                    <table class="table table-striped table-hover table-checkable order-column dataTable" id="dataTable">
                        <thead>
                            <tr>
                                ...
                                <?php echo Spr\Base\Controllers\Views\DataTable::headerCellTable($data, Lang::get('happy-code.name'),'','Name'); ?>
                            </tr>
                        </thead>
                        <tbody>
                            @if(isset($data))
                                @foreach($data['data']['response'] as $key => $item)
                                <tr class="odd gradeX" data-id="{{ $item->id }}" data-type="{{ $item->type }}" data-name="{{ $item->name }}" data-discount="{{ $item->discount }}" data-status="{{ $item->status }}">
                                    <td class=" sorting id">
                                        {{ $item->id }}
                                    </td>
                                    <td class="type">
                    ...
                    <div class="modal fade" id="myModal" role="dialog">
                        <div class="modal-dialog">
                            ...
                            <div class="modal-content">
                                <form action="{{ URL::Route('auth-post-delete-happy-code') }}" method="POST" class="form-delete">
        ...
        <div class="modal fade modal-form-data" id="modalEdit" role="dialog">
            <div class="modal-dialog">
                <!-- Modal content-->
                <div class="modal-content">
                    <form action="{{ URL::Route('auth-post-insert-update-happy-code') }}" method="POST" class="form-action form-edit" id="form-action-happy-code" method="POST">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-body">
                                <div class="form-group form-md-line-input">
                                    ...
                                    <label for="name">{{ Lang::get('happy-code.name') }}</label>
                                    <span class="help-block">{{ Lang::get('happy-code.help_name') }}</span>
                                </div>
                                <div class="form-group form-md-line-input">
                                    <select class="form-control" class="type" id="type" name="type">
                                        @foreach( Config::get('spr.system.happy_code_type') as $key => $value )
                                        <option value="{{ $key }}">{{ Lang::get('happy_code_type.' .  $value['title']) }}</option>
                                        @endforeach
                                    </select>
                                    <label for="type">{{ Lang::get('happy-code.type') }}</label>
                                    <span class="help-block">{{ Lang::get('happy-code.help_type') }}</span>
                                </div>
                                <div class="form-group form-md-line-input ">
                                ...
                                <div class="form-group form-md-line-input ">
                                    <select class="form-control" id="status" name="status">
                                        @foreach( Config::get('spr.system.happy_code_status') as $key => $value )
                                        <option value="{{ $key }}">{{ $value }}</option>
                                        @endforeach
                                    </select>
                                    <label for="status">{{ Lang::get('happy-code.status') }}</label>
                                    <span class="help-block">{{ Lang::get('happy-code.help_status') }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
