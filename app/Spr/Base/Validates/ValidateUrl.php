...
class ValidateUrl
{
	...
	public  function checkUrl($url){
		...
		if(filter_var($url,FILTER_VALIDATE_URL)){
				...
				if($flag){
					...
					if($this->checkDetailExist($url) == false){
						...
						if($this->getKeyword("field-keywords",$url) !=""){
							...
							$result['meta']['msg']		= "Liên kết phải là danh sách sản phẩm hoặc trang tìm kiếm của amazon.";
						}

					}else{
							$result['meta']['success']  = false;
							$result['meta']['code']	= 400;
							$result['meta']['msg']		= "Liên kết phải là đường dẫn chi tiết của sản phẩm trên amazon";
					}
					...
					if($flag){
						if($this->checkDetailExist($url) == false){
							...
							$result['meta']['success']  = false;
							$result['meta']['code']	= 400;
							$result['meta']['msg']		= "Liên kết phải là đường dẫn từ amazon.co.jp hoặc đường đãn của trang web. ";
					}
				...
				$result['meta']['success']  = false;
				$result['meta']['code']	= 400;
				$result['meta']['msg']		= "Liên kết này không hợp lệ.";
		}
		return $result;
	}
