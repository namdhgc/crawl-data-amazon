...
class Maintenance
{
     ...
     */
     ...
     public function setCacheTransactionStatus() {

          if(!Cache::has('transaction_status')){

               $ModelTransactionStatus = new ModelTransactionStatus();
               $dataTransactionStatus = $ModelTransactionStatus->getDataSelectBox();
               if(!empty($dataTransactionStatus['response'])) {

                    $lengthData = COUNT($dataTransactionStatus['response']);
                    $arrayData = array();

                    for ($i=0; $i < $lengthData; $i++) {

                         $arrayData[$dataTransactionStatus['response'][$i]->id] = $dataTransactionStatus['response'][$i]->name;
                    }

                    Cache::forever('transaction_status', $arrayData);
               }
          }
     }
          ...
          if(!Cache::has('module')){
               ...
               if(!empty($dataModelModule['response'])) {
                    ...
                    for ($i=0; $i < $lengthData; $i++) {
                         ...
                         $module_lang[$dataModelModule['response'][$i]->name] = [
                              'id' => $dataModelModule['response'][$i]->id,
                              'lang' => Lang::get('permissionRoles.'.$dataModelModule['response'][$i]->name)
                         ];
                    }
                    Cache::forever('module', $arrayData);
                    Cache::forever('module-lang', $arrayData);
               }
          }
     }
     ...
     public function setCacheWhyChooseUs() {

          if(!Cache::has('why_choose_us')){

               $data_why_choose_us =    $this->getSettingFollowKey(Config::get('spr.type.setting.why-choose-us.key'));

               if(!empty($data_why_choose_us['response'])) {

                    $lengthData = COUNT($data_why_choose_us['response']);
                    $arrayData = array();

                    for ($i=0; $i < $lengthData; $i++) {

                         $tmp = [];
                         $tmp['title']            =    $data_why_choose_us['response'][$i]->title;
                         $tmp['path']            =    $data_why_choose_us['response'][$i]->path;
                         $tmp['description']      =    $data_why_choose_us['response'][$i]->description;
                         array_push($arrayData, $tmp);
                    }

                    Cache::forever('why_choose_us', $arrayData);
               }
          }
     }
     ...
     public function setCacheTotalProducts() {

          if(!Cache::has('total_products')){

               $ModelSetting = new ModelSetting();
               $total_products = $this->getSettingFollowKey(Config::get('spr.type.setting.company-info.total_products.key'));
               
               if(!empty($total_products['response'])) {

                    $lengthData = COUNT($total_products['response']);
                    $data   = "";

                    for ($i=0; $i < $lengthData; $i++) {

                         $data = $total_products['response'][$i]->title;
                    }

                    Cache::forever('total_products', $data);
               }
          }
     }
